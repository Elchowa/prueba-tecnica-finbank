...
Generación de datos (Python) 
   ...
   S3: bronze/  (datos crudos + columnas de auditoría)
        │
        ▼
   Athena: finbank_silver  (limpieza, deduplicación, enmascaramiento)
        │
        ▼
   Athena: finbank_gold  (modelo dimensional + reglas de negocio)
```
...
1. Ejecutar `python data-generation/generate_data.py` para generar los CSV
...
- `ind_sospechoso`: se marca una transacción como sospechosa cuando su monto (`vr_mov`) supera en más de 3 desviaciones estándar el promedio de los últimos 30 días del mismo cliente.
